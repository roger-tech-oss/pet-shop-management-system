     ...
     <div class="container-fluid px-4 px-lg-5 mt-5">
        ...
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
            ...
            while ($row = $products->fetch_assoc()):
                ...
                $sql_avg = "SELECT AVG(rating) as average_rating FROM ratings WHERE product_id = ?";
                $stmt_avg = $conn->prepare($sql_avg);
                $stmt_avg->bind_param("i", $row['id']);
                $stmt_avg->execute();
                $result_avg = $stmt_avg->get_result();
                $row_avg = $result_avg->fetch_assoc();
                $average_rating = round($row_avg['average_rating'], 1); // Round to 1 decimal
                ?>
            <div class="col mb-5">
                <div class="card h-100 product-item">
                    <!-- Product image-->
                    ...
                    <div class="card-body p-4">
                        <div class="text-center">
                            ...
                            <p>Average Rating: <?php echo $average_rating ? $average_rating . ' / 5' : 'Not Rated'; ?></p>
                        </div>
                    </div>
                    <!-- Product actions-->
                    <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                        <div class="text-center">
                            <a class="btn btn-flat btn-primary" href=".?p=view_product&id=<?php echo md5($row['id']) ?>">View</a>
                            <a class="btn btn-flat btn-secondary" data-bs-toggle="modal" data-bs-target="#ratingModal-<?php echo $row['id']; ?>">Rate</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="ratingModalLabel">Rate <?php echo $row['product_name']; ?></h5>
